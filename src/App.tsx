...
function App() {
  ...
  const dimensionsLabel = formatDimensions(useMemo(
    ...
    <div className="min-h-screen bg-stone-100 text-zinc-900">
      ...
      <main className="px-4 py-7 sm:px-8 sm:py-10">
        <section className="mx-auto max-w-6xl" id="workspace" aria-label="GIFCAM 이미지 작업 영역">
          <CameraShell frames={frames} selectedFrame={selectedFrame} totalDurationMs={totalDurationMs} dimensionsLabel={dimensionsLabel} defaultDelayMs={defaultDelayMs} outputSettings={outputSettings} onAddFiles={addFiles} onOpenFilePicker={openFilePicker} onDefaultDelayChange={setDefaultDelayMs} onApplyDelayToAll={() => applyDelayToAll(defaultDelayMs)} onLoopModeChange={(loopMode: LoopMode) => setOutputSettings((current) => ({ ...current, loopMode }))} onSizePresetChange={(sizePreset: OutputSizePreset) => setOutputSettings((current) => ({ ...current, sizePreset }))} />
