import type { Dimensions, GifFrame, GifGenerationProgress, GifGenerationStatus, OutputSettings } from '../types/gif'
...
type CameraShellProps = {
  ...
  generationProgress: GifGenerationProgress | null
...
export function CameraShell({ frames, selectedFrame, totalDurationMs, dimensionsLabel, outputDimensions, defaultDelayMs, outputSettings, useCanvasPreview, generationStatus, generationProgress, hasCurrentResult, onAddFiles, onOpenFilePicker, onDefaultDelayChange, onApplyDelayToAll, onOutputSettingsChange, onVisualOutputSettingsChange, onGenerationAction }: CameraShellProps) {
  return (
    <>
      <div className="mx-auto flex min-h-11 w-[88%] items-end justify-between gap-3"><div className="flex items-end gap-3" aria-hidden="true"><span className="h-4 w-28 border-x-3 border-t-3 border-[#384a3d] bg-[#526556]" /><span className="h-3 w-14 border-x-3 border-t-3 border-zinc-500 bg-stone-100" /></div><ShutterControl status={generationStatus} progress={generationProgress} hasCurrentResult={hasCurrentResult} onAction={onGenerationAction} /></div>
