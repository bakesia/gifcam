...
export function CameraControls({ frameCount, totalDurationMs, dimensionsLabel, defaultDelayMs, outputSettings, onOpenFilePicker, onDefaultDelayChange, onApplyDelayToAll, onOutputSettingsChange, onVisualOutputSettingsChange }: CameraControlsProps) {
  ...
  return (
    <aside className="mt-4 grid gap-4 lg:mt-0 lg:flex lg:self-stretch lg:flex-col lg:justify-center" aria-label="출력 설정">
      ...
      {frameCount > 0 && <button type="button" className="inline-flex min-h-12 cursor-pointer items-center justify-center gap-2 border-2 border-[#455a4a] bg-white px-3 text-sm font-bold text-zinc-900 transition hover:-translate-y-0.5 hover:border-[#33483a] hover:bg-stone-50 hover:shadow-[0_2px_0_rgba(24,24,23,0.16)] focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-[#a64132]" onClick={onOpenFilePicker}><Plus size={18} /> 이미지 추가</button>}
      <div className="grid gap-3 border-t border-[#b7b3a6] pt-4">
        ...
        <button type="button" className="cursor-pointer border-2 border-[#455a4a] bg-[#526556] px-3 py-2 font-machine text-[12px] font-bold tracking-[0.05em] text-white transition hover:-translate-y-0.5 hover:border-[#33483a] hover:bg-[#384a3d] hover:shadow-[0_2px_0_#25342a] active:translate-y-0 focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-[#a64132]" onClick={onApplyDelayToAll}>모든 프레임에 적용</button>
